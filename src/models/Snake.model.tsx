...
export class Snake {
    ...
    isHead(point: BoardPoint): boolean {
        return this.points[0].row === point.row && this.points[0].column === point.column;
    }

    isBody(point: BoardPoint): boolean {
        return !!this.points.find((item, index) => item.row === point.row && item.column === point.column && index !== 0);
    }
